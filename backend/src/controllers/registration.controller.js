...
export const cancelRegistration = async (req, res) => {
    try {
        const eventId = req.params.eventId;
        const volunteerId = req.user._id;
        const result = await Registration.findOneAndDelete({ event: eventId, volunteer: volunteerId });
        if (!result) {
...
export const updateRegistrationStatus = async (req, res) => {
    try {
        const { status } = req.body; // 'approved' hoặc 'rejected'
        const updatedReg = await Registration.findByIdAndUpdate(req.params.registrationId, { status }, { new: true });
...
export const markAsCompleted = async (req, res) => {
    try {
        const updatedReg = await Registration.findByIdAndUpdate(req.params.registrationId, { status: 'completed' }, { new: true });
        res.status(200).json({ message: 'Đánh dấu hoàn thành thành công', registration: updatedReg });
    } catch (error) {
        res.status(500).json({ message: 'Lỗi server', error: error.message });
    }
};
